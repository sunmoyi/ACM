		...
		if (!pre[v])
		{
			...
			if (low[v] >= pre[u])
			{
				...
				while (true)
				{
					...
					if (bccno[x.v] != bcc_cnt)//不重复压入不定长数组
					{//bcc还可以用来统计点的个数
						bcc[bcc_cnt].push_back(x.v), bccno[x.v] = bcc_cnt;//bcc bcc_cnt这个双连通分量中包含的点
						belong[x.v].push_back(bcc_cnt);//bccno数组保存某个点属于哪个双连通分量
					}//belong不定长数组   x.v这个边属于那几个双连通分量
					if (x.u == u && x.v == v)
						break;
				}
			}
