...
struct LineB {
	...
	LineB() {}
	pair<int, Point> operator &(const LineB &k)const
	{
		Point res = a;
		if (dcmp((a - b) ^ (k.a - k.b)) == 0)
		{
			if (dcmp((a - k.b) ^ (k.a - k.b)) == 0)
				return make_pair(0, res);//重合
			else return make_pair(1, res);//平行
		}
		double t = ((a - k.a) ^ (k.a - k.b)) / ((a - b) ^ (k.a - k.b));
		res.x += (b.x - a.x)*t;
		res.y += (b.y - a.y)*t;
...
//判断直线l1和线段l2是否相交
bool Seg_inter_line(LineB l1, LineB l2) 
{
	return dcmp((l2.a - l1.b) ^ (l1.a - l1.b))* dcmp((l2.b - l1.b) ^ (l1.a - l1.b)) <= 0;
}
...
int main(void)
{
	...
	while (T--)
	{
		...
		l1 = LineB(Point(x1, y1), Point(x2, y2));
		l2 = LineB(Point(x3, y3), Point(x4, y4));
		if (dcmp(l1.a.y - l1.b.y) == 0 || dcmp(l2.a.y - l2.b.y) == 0)
		{
			printf("0.00\n");
			continue;
		}
		if (dcmp(l1.a.y - l1.b.y) < 0)
			swap(l1.a, l1.b);
		if (dcmp(l2.a.y - l2.b.y) < 0)
			swap(l2.a, l2.b);
		...
		if (inter(LineB(l1.a, Point(l1.a.x, 100000)), l2))
		{
			printf("0.00\n");
			continue;
		}
		if (inter(LineB(l2.a, Point(l2.a.x, 1000000)), l1))
		{
			printf("0.00\n");
			continue;
		}
		...
		double ans1;
		pr = l1 & LineB(Point(1000000, l2.a.y), l2.a);
		Point p1 = pr.second;

		double ans2;
		pr = l2 & LineB(Point(100000, l1.a.y), l1.a);
		Point p2 = pr.second;
		ans2 = fabs((l1.a - p) ^ (p2 - p)) / 2;
		printf("%.2f\n", min(ans1, ans2));
	}
	return 0;
}