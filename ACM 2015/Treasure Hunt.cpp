...
int main(void)
{
	...
	while (scanf("%d", &n) != EOF)
	{
		for (int i = 1; i <= n; i++)
		{
			...
			line[i] = LineB(Point(x1, y1), Point(x2, y2));
			p[2 * i - 1] = Point(x1, y1);
			p[2 * i] = Point(x2, y2);
		}
		scanf("%lf %lf", &x1, &y1);
