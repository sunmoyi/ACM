...
int main()
{
	...
	while (t--) {
		...
		if (n < 3) flag = true;
		for (i = 0; i < n && !flag; i++) {
			if (Check(LineB(line[i].a, line[i].b))) flag = true;
			for (j = i + 1; j < n && !flag; j++) {
				if (Check(LineB(line[i].a, line[j].a)) ||
					Check(LineB(line[i].a, line[j].b)) ||
					Check(LineB(line[i].b, line[j].a)) ||
					Check(LineB(line[i].b, line[j].b)))
					flag = true;
			}
		}
		if (flag) printf("Yes!\n");
