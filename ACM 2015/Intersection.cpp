...
int main (void)
{
	...
	for (int i = 1; i <= T; i++)
	{
		scanf("%lf %lf %lf %lf", &x1, &yx, &x2, &y2);
		linee = LineB(Point(x1, yx), Point(x2, y2));
		px = Point(x1, yx);
		py = Point(x2, y2);
		scanf("%lf %lf %lf %lf", &x1, &yx, &x2, &y2);
		p1 = Point(min(x1, x2), min(yx, y2));
		p2 = Point(max(x1, x2), min(yx, y2));
		p3 = Point(max(x1, x2), max(yx, y2));
		p4 = Point(min(x1, x2), max(yx, y2));
		if (inter(linee, LineB(p1, p2)))
		{
			printf("T\n");
			continue;
		}
		if (inter(linee, LineB(p2, p3)))
		{
			printf("T\n");
			continue;
		}
		if (inter(linee, LineB(p3, p4)))
		{
			printf("T\n");
			continue;
		}
		if (inter(linee, LineB(p4, p1)))
		{
			printf("T\n");
			continue;
		}
		...
		ploy.push_back(p3);
		ploy.push_back(p4);
		if (IsPointInPolygon(px, ploy) != 0 && IsPointInPolygon(py, ploy) != 0)
		{
			printf("T\n");
			continue;
		}
